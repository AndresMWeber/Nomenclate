...
class TestTokenAttrBase(basetest.TestBase):
    def setUp(self):
        ...
        self.token_attr_dict_handler = tokens.TokenAttrDictHandler(self.nomenclate.token_dict.to_json())
...
class TestPurge(TestTokenAttrBase):
    def test_default(self):
        handler = tokens.TokenAttrDictHandler(self.nomenclate.state)
...
class TestEq(TestTokenAttrBase):
    def test_not_equal(self):
        other_token_dict_handler = tokens.TokenAttrDictHandler(nm.Nom().token_dict.to_json())
        other_token_dict_handler.merge_json({'name': 'bob'})
        self.assertFalse(self.token_attr_dict_handler == other_token_dict_handler)

    def test_equal(self):
        self.assertTrue(self.token_attr_dict_handler == tokens.TokenAttrDictHandler(nm.Nom().token_dict.to_json()))
